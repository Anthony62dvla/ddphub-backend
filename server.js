// --- START DEBUG CODE FOR RENDER DEPLOYMENT ---
console.log('\n--- Render Deployment Debugging ---');
console.log('Timestamp:', new Date().toISOString());
console.log('Process ID:', process.pid);
console.log('Current working directory (process.cwd()):', process.cwd());
console.log('__dirname (module directory):', __dirname);
...
try {
    ...
    fs.readdirSync(process.cwd()).forEach(item => {
        console.log(`- ${item} (${fs.statSync(path.join(process.cwd(), item)).isDirectory() ? 'DIR' : 'FILE'})`);
    });
} catch (e) {
...
try {
    ...
    console.log(`Is node_modules a directory: ${stat.isDirectory()}`);
    ...
    if (stat.isDirectory()) {
        ...
        try {
            const expressStat = fs.statSync(expressPath);
            console.log(`\nExpress module exists at: ${expressPath}`);
            console.log(`Is Express module a directory: ${expressStat.isDirectory()}`);
        } catch (e) {
            console.log(`Express module NOT found at ${expressPath}: ${e.message}`);
        }
    }
} catch (e) {
    console.error(`node_modules NOT found at ${nodeModulesPath}: ${e.message}`);
}
...
console.log('\n--- Attempting to require express (after checks) ---');
// --- END DEBUG CODE FOR RENDER DEPLOYMENT ---

const express = require('express'); // This is the line that errors out
